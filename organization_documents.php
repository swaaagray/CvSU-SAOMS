...
$orgType = 'new';
// Fetch organization type to conditionally show required documents
...
$submissionStatus = checkSubmissionPeriod($orgId, 'organization');
...
try {
    ...
    if (!empty($activeAcademicYearId)) {
        ...
        $selectStmt = $conn->prepare("SELECT id, file_path FROM organization_documents WHERE organization_id = ? AND (academic_year_id IS NULL OR academic_year_id <> ?)");
        if ($selectStmt) {
            $selectStmt->bind_param("ii", $orgId, $activeAcademicYearId);
        ...
        $deleteStmt = $conn->prepare("DELETE FROM organization_documents WHERE organization_id = ? AND (academic_year_id IS NULL OR academic_year_id <> ?)");
        if ($deleteStmt) {
            $deleteStmt->bind_param("ii", $orgId, $activeAcademicYearId);
            $deleteStmt->execute();
        }
    }
} catch (Exception $e) {
    error_log('Organization documents cleanup failed: ' . $e->getMessage());
}
        ...
        switch ($_POST['action']) {
            case 'upload':
                ...
                if (!empty($document_type)) {
                    ...
                    $submissionCheck = checkSubmissionPeriod($orgId, 'organization');
                    if (!$submissionCheck['can_submit']) {
                        $_SESSION['error'] = $submissionCheck['message'];
                        header('Location: organization_documents.php');
                        exit;
                    }
                    
                    // Prevent duplicate submissions for the same document type
                    $existsStmt = $conn->prepare("SELECT 1 FROM organization_documents WHERE organization_id = ? AND document_type = ? LIMIT 1");
                    $existsStmt->bind_param("is", $orgId, $document_type);
                    $existsStmt->execute();
                    ...
                    if (!empty($_FILES['document']['name'])) {
                        // Determine academic term id with fallback to organization's stored term
                        ...
                        if (in_array($file_ext, ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png', 'xls', 'xlsx', 'csv'])) {
                            ...
                            if (move_uploaded_file($_FILES['document']['tmp_name'], $upload_path)) {
                                $stmt = $conn->prepare("INSERT INTO organization_documents (organization_id, academic_year_id, document_type, file_path, submitted_by) VALUES (?, ?, ?, ?, ?)");
                                $stmt->bind_param("iissi", $orgId, $currentAcademicTermId, $document_type, $upload_path, $_SESSION['user_id']);
                                ...
                                if ($stmt->execute()) {
                                    ...
                                    require_once 'includes/notification_helper.php';
                                    notifyDocumentSubmission($orgId, $document_type, $documentId);
                                    
                                    // New/updated submission means organization cannot be recognized yet
                                    $statusStmt = $conn->prepare("UPDATE organizations SET status = 'unrecognized' WHERE id = ?");
                            ...
                            $_SESSION['error'] = 'Invalid file type. Please upload PDF, DOC, DOCX, JPG, JPEG, or PNG files.';
                        }
                    } else {
            ...
            case 'delete':
                ...
                if ($documentId) {
                    ...
                    $stmt = $conn->prepare("SELECT file_path FROM organization_documents WHERE id = ? AND organization_id = ?");
                    $stmt->bind_param("ii", $documentId, $orgId);
                    $stmt->execute();
                    ...
                    $stmt = $conn->prepare("DELETE FROM organization_documents WHERE id = ? AND organization_id = ?");
                    $stmt->bind_param("ii", $documentId, $orgId);
                    if ($stmt->execute()) {
                        $_SESSION['message'] = 'Document deleted successfully!';
                        // Deletion means organization cannot be recognized
                        $statusStmt = $conn->prepare("UPDATE organizations SET status = 'unrecognized' WHERE id = ?");
                        $statusStmt->bind_param("i", $orgId);
                        $statusStmt->execute();
                    } else {
                ...
                if (!empty($documentType) && !empty($_FILES['new_document']['name'])) {
                    ...
                    if (in_array($file_ext, ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png'])) {
                        ...
                        if (move_uploaded_file($_FILES['new_document']['tmp_name'], $upload_path)) {
                            ...
                            $stmt = $conn->prepare("UPDATE organization_documents SET file_path = ?, status = 'pending', submitted_at = NOW(), adviser_approved_at = NULL, adviser_rejected_at = NULL, osas_approved_at = NULL, osas_rejected_at = NULL, approved_by_adviser = NULL, approved_by_osas = NULL, rejection_reason = NULL, academic_year_id = IFNULL(academic_year_id, ?) WHERE id = ? AND organization_id = ?");
                            $stmt->bind_param("siii", $upload_path, $currentAcademicTermId, $documentId, $orgId);
                            ...
                            if ($stmt->execute()) {
                                ...
                                // Notify adviser and OSAS about resubmission (treated as a new submission)
                                require_once 'includes/notification_helper.php';
...
// Get organization's documents with academic year filtering (ACTIVE YEARS ONLY)
$query = "
    SELECT d.*, 
           ...
           COALESCE(at_doc.status, at_org.status) as academic_year_status,
           COALESCE(at_doc.school_year, at_org.school_year) as school_year
...
// If organization is NEW, hide optional legacy documents (not required)
if ($orgType === 'new') {
...
<head>
    ...
    <title>Organization Documents - Academic Organization Management System</title>
    ...
    <main>
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="page-title">Organization Documents</h2>
                    <p class="page-subtitle">Upload and manage required organization documents</p>
                </div>
                <div>
                    <?php if ($canSubmit): ?>
                                    ...
                                    <?php foreach ($documents as $document): 
                                        ...
                                        if ($notificationId && $notificationType && isset($document['id'])) {
                                            ...
                                            elseif ($notificationType === 'documents_sent_to_osas') {
                                                ...
                                                if ($document['status'] === 'approved' && !empty($document['adviser_approved_at']) && empty($document['osas_approved_at'])) {
                                                    $isHighlighted = true;
                                                }
                                            }
                                        }
                                        ...
                                        <tr class="<?php echo $isHighlighted ? 'notification-highlight' : ''; ?>" 
                                            ...
                                            <td>
                                                <?php if ($document['status'] === 'rejected' && !empty($document['rejection_reason'])): ?>
                                                    <button type="button" class="btn btn-sm btn-primary" onclick="openResubmitModal(<?php echo $document['id']; ?>, '<?php echo addslashes($document['rejection_reason']); ?>', '<?php echo addslashes($document['file_path']); ?>', '<?php echo addslashes($document['document_type']); ?>')">
                ...
                <form method="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <?php
                                // Build a set of already-submitted types for this organization
