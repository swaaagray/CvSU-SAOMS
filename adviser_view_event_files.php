...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        switch ($_POST['action']) {
            case 'approve':
                // Get document type and organization info first
                $stmt = $conn->prepare("
                    SELECT ed.document_type, ed.event_approval_id, ea.organization_id, ea.title 
                ...
                if ($document) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        notifyAdviserEventAction($document['organization_id'], 'approved', $document['document_type'], $documentId);
                        ...
                        if ($allApproved['total'] > 0 && $allApproved['approved'] == $allApproved['total']) {
                            ...
                            if ($eventDetails) {
                                notifyEventDocumentsSentToOSAS($document['organization_id'], $eventDetails['title'], $eventDetails['event_approval_id']);
                            }
                        }
                ...
                if (empty($rejectionReason)) {
                    ...
                    if ($document) {
                        ...
                        if ($stmt->execute()) {
                            // Create notification for president
                            require_once 'includes/notification_helper.php';
                            notifyAdviserEventAction($document['organization_id'], 'rejected', $document['document_type'], $documentId);
        ...
        header('Location: adviser_view_event_files.php');
        exit;
    }
}
...
$query = "
    ...
    WHERE ea.organization_id = ? AND ea.organization_id IS NOT NULL
";
...
<head>
    ...
    <title>Event Files - Organization Adviser</title>
    ...
    <style>
        ...
        /* Filter section styling - matching organization documents */
        .filter-section {
        ...
        /* Form styling - matching organization documents */
        .form-label {
            color: #495057;
            font-weight: 600;
            margin-bottom: 0.5rem;
...
<body>
    ...
    <main>
        <div class="container-fluid py-2">
            ...
            <div class="page-header">
                ...
                <p class="page-subtitle">Review and manage event proposal documents for <strong><?php echo htmlspecialchars($organization['org_name']); ?></strong></p>
            </div>
                        ...
                        <div class="card-body">
                            <form method="GET" action="">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="adviser_view_event_files.php" class="btn btn-secondary">
    ...
    <script>
        // Global viewer used by injected event details to open documents
        window.viewEventDoc = function(filePath, title) {
            // Create or reuse a shared viewer modal
            ...
            if (["jpg","jpeg","png","gif","bmp","webp"].includes(ext)) {
                ...
                if (typeof CustomPDFViewer !== 'undefined') {
                    ...
                    // Lazy-load assets then initialize embedded viewer
        ...
        // Cache and restore approve/reject modals to allow continuous actions
        ...
        function refreshListsAndDetails() {
            ...
            setTimeout(() => {
                fetch('adviser_view_event_files.php')
                    .then(r => r.text())
                    .then(html => {
        ...
        // Handle approve/reject via delegated submit so handler survives DOM changes
                ...
                if (txt.includes('successfully')) {
                    ...
                    body.innerHTML = `
                        <div class="text-center">
                            ...
                            <h5 class="mt-3">${isApprove ? 'Document Approved Successfully!' : 'Document Rejected Successfully!'}</h5>
        ...
        // (Direct reject handler removed in favor of delegated handler above)
