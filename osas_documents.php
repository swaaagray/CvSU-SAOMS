...
$pageOrg = isset($_GET['page_org']) ? max(1, (int)$_GET['page_org']) : 1;
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'approve_org_doc':
                ...
                if ($docId) {
                    $stmt = $conn->prepare("UPDATE organization_documents SET status = 'approved', osas_approved_at = NOW(), approved_by_osas = ? WHERE id = ?");
                    $stmt->bind_param("ii", $_SESSION['user_id'], $docId);
                    if ($stmt->execute()) {
                        // Get document details for notification
                        $docStmt = $conn->prepare("SELECT organization_id, document_type FROM organization_documents WHERE id = ?");
                        ...
                        if ($docDetails) {
                            ...
                            notifyDocumentAction($docDetails['organization_id'], 'approved', $docDetails['document_type'], $docId);
                        }
                        
                        // Recalculate organization recognition status based on required documents for org type
                        if ($docDetails && !empty($docDetails['organization_id'])) {
                            $orgId = $docDetails['organization_id'];

                            // Determine organization type
                            $orgType = 'new';
                            $typeStmt = $conn->prepare("SELECT type FROM organizations WHERE id = ?");
                            if ($typeStmt) {
                                $typeStmt->bind_param("i", $orgId);
                                if ($typeStmt->execute()) {
                            ...
                            $query = "SELECT COUNT(*) AS approved_count FROM organization_documents WHERE organization_id = ? AND document_type IN ($placeholders) AND status = 'approved'";
                            $stmt = $conn->prepare($query);
                            $approvedCount = 0;
                            if ($stmt) {
                                $bindParams = [];
                                $bindTypes = 'i' . str_repeat('s', count($requiredDocuments));
                                $bindValues = array_merge([$orgId], $requiredDocuments);
                            ...
                            if ($approvedCount === count($requiredDocuments)) {
                                $stmt = $conn->prepare("UPDATE organizations SET status = 'recognized' WHERE id = ?");
                                $stmt->bind_param("i", $orgId);
                                $stmt->execute();
                            } else {
                                $stmt = $conn->prepare("UPDATE organizations SET status = 'unrecognized' WHERE id = ?");
                                $stmt->bind_param("i", $orgId);
                                $stmt->execute();
                            }
                        }
            ...
            case 'reject_org_doc':
                ...
                if ($docId && !empty($reason)) {
                    ...
                    if (!empty($deadline)) {
                        $stmt = $conn->prepare("UPDATE organization_documents SET status = 'rejected', osas_rejected_at = NOW(), approved_by_osas = ?, rejection_reason = ?, resubmission_deadline = ?, deadline_set_by = ?, deadline_set_at = NOW() WHERE id = ?");
                        $stmt->bind_param("issii", $_SESSION['user_id'], $reason, $deadline, $_SESSION['user_id'], $docId);
                    } else {
                        $stmt = $conn->prepare("UPDATE organization_documents SET status = 'rejected', osas_rejected_at = NOW(), approved_by_osas = ?, rejection_reason = ? WHERE id = ?");
                    ...
                    if ($stmt->execute()) {
                        ...
                        if ($docDetails) {
                            require_once 'includes/notification_helper.php';
                            notifyDocumentAction($docDetails['organization_id'], 'rejected', $docDetails['document_type'], $docId);
                        }
                        
                        // Any rejection guarantees the organization remains UNRECOGNIZED
                        if (!empty($docDetails['organization_id'])) {
                            $orgId = (int)$docDetails['organization_id'];
                            $stmt = $conn->prepare("UPDATE organizations SET status = 'unrecognized' WHERE id = ?");
                            $stmt->bind_param("i", $orgId);
                            $stmt->execute();
                        }
                ...
                if ($docId) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $eventStmt = $conn->prepare("
                            SELECT ea.organization_id, ea.title, ea.id as event_approval_id
                        ...
                        if ($eventDetails) {
                            require_once 'includes/notification_helper.php';
                            notifyEventAction($eventDetails['organization_id'], 'approved', $eventDetails['title'], $eventDetails['event_approval_id']);
                        }
            ...
            case 'reject_event_doc':
                ...
                if ($docId && !empty($reason)) {
                    ...
                    if ($stmt->execute()) {
                        ...
                        $eventStmt = $conn->prepare("
                            SELECT ea.organization_id, ea.title, ea.id as event_approval_id, ed.document_type
                            FROM event_documents ed 
                            JOIN event_approvals ea ON ed.event_approval_id = ea.id 
                            WHERE ed.id = ?
                        ");
                        ...
                        if ($eventDetails) {
                            require_once 'includes/notification_helper.php';
                            notifyOSASEventDocumentAction($eventDetails['organization_id'], $eventDetails['document_type'], $docId, $eventDetails['title']);
                        }
...
$organizations = $conn->query("SELECT id, org_name FROM organizations ORDER BY org_name")->fetch_all(MYSQLI_ASSOC);

// Build organization documents query - show documents that need OSAS attention
$wherePending = "d.adviser_approved_at IS NOT NULL AND d.osas_approved_at IS NULL AND d.osas_rejected_at IS NULL";
$orgDocsQueryBase = "
    SELECT d.*, o.org_name as org_name, u.username as submitted_by,
           d.adviser_approved_at, d.osas_approved_at, d.osas_rejected_at,
    ...
    LEFT JOIN academic_terms at_org ON o.academic_year_id = at_org.id
    JOIN users u ON d.submitted_by = u.id
    WHERE d.adviser_approved_at IS NOT NULL
      AND COALESCE(at_doc.status, at_org.status) = 'active'
";
...
if ($documentType) {
    $orgDocsQueryBase .= " AND d.document_type = ?";
    $params[] = $documentType;
    $types .= "s";
}

if ($organization) {
    $orgDocsQueryBase .= " AND d.organization_id = ?";
    $params[] = $organization;
    $types .= "i";
}

// For NEW organizations, hide year-2+ documents on OSAS side during the active academic year
// Only show the 11 required documents; the 3 additional appear when org becomes 'old'
$orgDocsQueryBase .= " AND (o.type <> 'new' OR d.document_type NOT IN ('accomplishment_report','previous_plan_of_activities','financial_report'))";

// Apply OSAS status filter within adviser-approved set
if ($status === 'pending') {
    $orgDocsQueryBase .= " AND d.osas_approved_at IS NULL AND d.osas_rejected_at IS NULL";
} elseif ($status === 'approved') {
    $orgDocsQueryBase .= " AND d.osas_approved_at IS NOT NULL";
} elseif ($status === 'rejected') {
    $orgDocsQueryBase .= " AND d.osas_rejected_at IS NOT NULL";
}
...
if ($startDate) {
    $orgDocsQueryBase .= " AND d.submitted_at >= ?";
...
// Count total org docs for pagination (avoid duplicate column names in derived table)
// Build COUNT query by replacing SELECT list with COUNT(*) while keeping same FROM/JOIN/WHERE
$orgCountQuery = "SELECT COUNT(*) as total " . substr($orgDocsQueryBase, strpos($orgDocsQueryBase, 'FROM'));
$stmt = $conn->prepare($orgCountQuery);
if (!empty($params)) {
    $stmt->bind_param($types, ...$params);
}
$stmt->execute();
$orgTotal = (int)($stmt->get_result()->fetch_assoc()['total'] ?? 0);
$orgTotalPages = max(1, (int)ceil($orgTotal / $perPage));
$orgOffset = ($pageOrg - 1) * $perPage;

// Fetch paginated org docs
$orgDocsQuery = $orgDocsQueryBase . " ORDER BY d.submitted_at DESC LIMIT ? OFFSET ?";
$stmt = $conn->prepare($orgDocsQuery);
if (!empty($params)) {
    $stmt->bind_param($types . "ii", ...array_merge($params, [$perPage, $orgOffset]));
} else {
    $stmt->bind_param("ii", $perPage, $orgOffset);
}
$stmt->execute();
$orgDocs = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Build event documents query - group by event
$eventDocsQueryBase = "
    SELECT ea.id as event_approval_id, ea.title as event_title, o.org_name as org_name,
...
if ($organization) {
    $eventDocsQueryBase .= " AND ea.organization_id = ?";
    $params[] = $organization;
    $types .= "i";
}
...
$eventGroupSection = " GROUP BY ea.id, ea.title, o.org_name, ea.created_at 
...
// Count total event groups (avoid duplicate column names)
$eventCountQuery = "SELECT COUNT(*) as total FROM (" . ($eventDocsQueryBase . $eventGroupSection) . ") t";
...
<head>
    ...
    <title>OSAS Documents - OSAS Portal</title>
    ...
    <style>
        ...
        /* Organization documents table - gray theme */
        ...
        /* Event documents table - darker gray theme */
        ...
        /* Responsive column widths for Organization Documents table */
        main .card:nth-of-type(2) .table th:nth-child(1), /* Organization */
        ...
        /* Responsive column widths for Event Documents table */
        ...
        main .card:nth-of-type(3) .table th:nth-child(2), /* Organization */
        ...
        @media (max-width: 768px) {
            ...
            /* Event documents table mobile adjustments */
        ...
        @media (max-width: 576px) {
            ...
            main .table {
                font-size: 0.7rem;
                min-width: 1000px; /* Ensure all columns visible with scroll */
            }
            ...
            /* Extra visible scrollbar on small screens */
...
<body>
    ...
    <main>
        <div class="container-fluid">
            <div class="page-header">
                <h2 class="page-title">OSAS Organization Documents</h2>
                <p class="page-subtitle">Manage and review organization documents</p>
            </div>
                ...
                <div class="card-body">
                    <form method="GET" class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <a href="osas_documents.php" class="btn btn-secondary">
            ...
            <div class="card mb-4">
                <div class="card-header">
                    <h3 class="card-title h5 mb-0">Organization Documents</h3>
                </div>
                <div class="card-body">
                    <?php if (empty($orgDocs)): ?>
                        <p class="text-muted">No organization documents approved by advisers found.</p>
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php foreach ($orgDocs as $doc): 
                                        // Check if this document should be highlighted based on notification
                                        $isHighlighted = false;
                                        if ($notificationId && $notificationType) {
                                            // For documents_for_review notifications, highlight the specific document that was sent for review
                                            if ($notificationType === 'documents_for_review' && $notificationId == $doc['id']) {
                                                $isHighlighted = true;
                                            }
                                            // For document_approved notifications, highlight the specific document that was approved
                                            elseif ($notificationType === 'document_approved' && $notificationId == $doc['id']) {
                                                $isHighlighted = true;
                                            }
                                            // For document_rejected notifications, highlight the specific document that was rejected
                                            elseif ($notificationType === 'document_rejected' && $notificationId == $doc['id']) {
                                                $isHighlighted = true;
                                            }
                                            // For document_submitted notifications, highlight the newly submitted document
                                    ...
                                    ?>
                                        <tr class="<?php echo $isHighlighted ? 'notification-highlight' : ''; ?>" 
                                            ...
                                            data-doc-title="<?php echo addslashes($documentTypes[$doc['document_type']]); ?>">
                                            <td><?php echo htmlspecialchars($doc['org_name']); ?></td>
                                            <td><?php echo $documentTypes[$doc['document_type']]; ?></td>
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-sm btn-info" 
                                                    onclick="viewFileModal('./<?php echo addslashes($doc['file_path']); ?>', '<?php echo addslashes($documentTypes[$doc['document_type']]); ?>')">
                        ...
                        <?php if ($orgTotalPages > 1): ?>
                        <nav class="mt-3">
                            <ul class="pagination pagination-sm">
                                <?php 
                                $qs = $_GET; 
                                for ($p = 1; $p <= $orgTotalPages; $p++): 
                                    $qs['page_org'] = $p;
                                    $url = 'osas_documents.php?' . http_build_query($qs);
                                ?>
                                    <li class="page-item <?php echo $p == $pageOrg ? 'active' : ''; ?>">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title h5 mb-0">Event Documents</h3>
                </div>
                <div class="card-body">
                    <?php if (empty($eventDocs)): ?>
                        <p class="text-muted">No pending event documents.</p>
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                ...
                                <tbody>
                                    <?php foreach ($eventDocs as $event): 
                                        ...
                                        $isHighlighted = false;
                                        if ($notificationId && $notificationType) {
                                            // For event_documents_for_review notifications, highlight the specific event that was sent for review
                                            if ($notificationType === 'event_documents_for_review' && $notificationId == $event['event_approval_id']) {
                                                $isHighlighted = true;
                                            }
                                            // For event_approved notifications, highlight the specific event that was approved
                                            elseif ($notificationType === 'event_approved' && $notificationId == $event['event_approval_id']) {
                                                $isHighlighted = true;
                                            }
                                            // For event_rejected notifications, highlight the specific event that was rejected
                                            elseif ($notificationType === 'event_rejected' && $notificationId == $event['event_approval_id']) {
                                                $isHighlighted = true;
                                            }
                                        }
                                    ?>
    ...
    <!-- Approve Organization Document Modal -->
    <div class="modal fade" id="approveOrgDocModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST">
                    <div class="modal-header">
                        <h5 class="modal-title">Approve Organization Document</h5>
    ...
    <!-- Reject Organization Document Modal -->
    ...
    <div class="modal fade" id="approveEventDocModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Approve Event Document</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p>Are you sure you want to approve this document?</p>
    ...
    <div class="modal fade" id="eventDocsModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="eventDocsModalTitle">Event Documents</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
    ...
    <script>
        ...
        document.getElementById('eventDocsModal').addEventListener('show.bs.modal', function(event) {
            ...
            document.getElementById('eventDocsModalTitle').textContent = 'Event Documents';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (notificationTab) {
                let targetSection;
                if (notificationTab === 'organization_documents') {
                    targetSection = document.querySelector('.card:has(h3:contains("Organization Documents"))');
                } else if (notificationTab === 'event_documents') {
                    targetSection = document.querySelector('.card:has(h3:contains("Event Documents"))');
                }
        ...
        function submitOrgDocAction(action, docId, reason = '', deadline = '') {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const modal = isApprovalForm ? document.getElementById('approveOrgDocModal') : document.getElementById('rejectOrgDocModal');
                    const body = modal.querySelector('.modal-body');
                    body.innerHTML = '<div class="alert alert-danger"><i class="bi bi-exclamation-triangle me-2"></i>' + (data.message || 'An error occurred while processing the request.') + '</div>';
                }
            })
            .catch(error => {
                // Show error message within the modal
        ...
        function submitEventDocAction(action, docId, reason = '', deadline = '') {
            ...
            // Find the submit button and add loading effect
            const approvalModal = document.querySelector('#approveEventDocModal');
            const rejectionModal = document.querySelector('#rejectEventDocModal');
            const activeModal = approvalModal && approvalModal.classList.contains('show') ? approvalModal : rejectionModal;
            const submitButton = activeModal ? activeModal.querySelector('button[onclick*="handleEventDoc"]') : null;
            let originalText = '';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Organization document rejection
