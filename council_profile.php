...
// Get council ID from URL
$council_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

// Get council details
$stmt = $conn->prepare("
    SELECT c.*, co.name as college_name, co.id as college_id
    FROM council c
...
$stmt = $conn->prepare("SELECT COUNT(*) as total FROM events WHERE council_id = ?");
$stmt->bind_param("i", $council_id);
$stmt->execute();
$total_events = $stmt->get_result()->fetch_assoc()['total'];

// Get total count of awards
$stmt = $conn->prepare("SELECT COUNT(*) as total FROM awards WHERE council_id = ?");
$stmt->bind_param("i", $council_id);
$stmt->execute();
...
// Get council's events (paginated)
$stmt = $conn->prepare("
    SELECT e.*, 
    ...
    LIMIT ? OFFSET ?
");
$stmt->bind_param("iii", $council_id, $items_per_page, $events_offset);
$stmt->execute();
$events = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Get council's awards (paginated)
$stmt = $conn->prepare("
    SELECT a.*, 
    ...
    LIMIT ? OFFSET ?
");
$stmt->bind_param("iii", $council_id, $items_per_page, $awards_offset);
$stmt->execute();
$awards = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Get council officers including advisers (current active academic year)
$stmt = $conn->prepare("
    ...
    WHERE so.council_id = ? AND at.status = 'active'
    UNION ALL
    SELECT COALESCE(c.adviser_name, u.email) as name, '' as course, '' as year_section,
    ...
    WHERE ao.council_id = ? AND at2.status = 'active'
    ORDER BY 
        CASE LOWER(position)
            WHEN 'adviser' THEN 1
        ...
        name ASC
");
$stmt->bind_param("ii", $council_id, $council_id);
$stmt->execute();
$members = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Get council adviser from adviser_officials table
$stmt = $conn->prepare("
    SELECT ao.*, u.email as adviser_email, c.adviser_name
    FROM adviser_officials ao
    JOIN users u ON ao.adviser_id = u.id
    JOIN academic_terms at ON ao.academic_year_id = at.id
    LEFT JOIN council c ON ao.council_id = c.id
    WHERE ao.council_id = ? AND at.status = 'active'
");
$stmt->bind_param("i", $council_id);
$stmt->execute();
$advisers = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Note: Advisers are not displayed in the officers section for student view

// No hierarchy on student side: render a flat list

// Build absolute public URL for assets under pretty URLs
...
<head>
    ...
    <title><?php echo htmlspecialchars($council['council_name']); ?> - Council Profile</title>
    ...
    <style>
        ...
        .award-card:hover {
            ...
            box-shadow: 0 6px 18px rgba(122,90,0,0.12);
            transform: translateY(-2px);
        }
        .award-card h6 {
            color: #1877f2;
...
<body>
            ...
            <div class="col-md-8 mx-auto">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="row align-items-center">
                            <?php if (!empty($council['logo_path'])): ?>
                            <div class="col-md-3 text-center mb-3 mb-md-0">
                                <img src="<?php echo htmlspecialchars($toPublicUrl($council['logo_path'])); ?>" 
                                     alt="<?php echo htmlspecialchars($council['council_name']); ?> Logo" 
                                     class="img-fluid rounded-circle" 
                                     style="max-width: 150px; height: auto;">
                            </div>
                            <?php endif; ?>
                            <div class="col-md-<?php echo !empty($council['logo_path']) ? '9' : '12'; ?>">
                                <h2 class="mb-2"><?php echo htmlspecialchars($council['council_name']); ?></h2>
                                <p class="text-muted mb-2">
                                    <i class="bi bi-building me-1"></i>
                                    <?php echo htmlspecialchars($council['college_name']); ?>
                                </p>
                                <?php if (!empty($council['description'])): ?>
                                    <p class="mb-2"><?php echo htmlspecialchars($council['description']); ?></p>
                                <?php endif; ?>
                                <?php if (!empty($council['facebook_link'])): ?>
                                    <a href="<?php echo htmlspecialchars($council['facebook_link']); ?>" 
                                       target="_blank" 
                    ...
                    <div class="card-body" id="events-container">
                        <?php if (empty($events)): ?>
                            <div class="text-center p-4">
                                <p class="text-muted mb-0">No events found for this council.</p>
                            </div>
                        <?php else: ?>
                            <div class="row">
                                ...
                                <nav aria-label="Events pagination" class="mt-3">
                                    <ul class="pagination justify-content-center mb-0">
                                        <li class="page-item <?php echo $events_page <= 1 ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?id=<?php echo $council_id; ?>&events_page=<?php echo $events_page - 1; ?>&awards_page=<?php echo $awards_page; ?>">Previous</a>
                                        </li>
                                        <?php for ($i = 1; $i <= $total_events_pages; $i++): ?>
                                            <li class="page-item <?php echo $events_page == $i ? 'active' : ''; ?>">
                                                <a class="page-link" href="?id=<?php echo $council_id; ?>&events_page=<?php echo $i; ?>&awards_page=<?php echo $awards_page; ?>"><?php echo $i; ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?php echo $events_page >= $total_events_pages ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?id=<?php echo $council_id; ?>&events_page=<?php echo $events_page + 1; ?>&awards_page=<?php echo $awards_page; ?>">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            <?php endif; ?>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body" id="awards-container">
                        <?php if (empty($awards)): ?>
                            <div class="text-center p-4">
                                <p class="text-muted mb-0">No awards found for this council.</p>
                            </div>
                        <?php else: ?>
                            <div class="row">
                                ...
                                <nav aria-label="Awards pagination" class="mt-3">
                                    <ul class="pagination justify-content-center mb-0">
                                        <li class="page-item <?php echo $awards_page <= 1 ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?id=<?php echo $council_id; ?>&events_page=<?php echo $events_page; ?>&awards_page=<?php echo $awards_page - 1; ?>">Previous</a>
                                        </li>
                                        <?php for ($i = 1; $i <= $total_awards_pages; $i++): ?>
                                            <li class="page-item <?php echo $awards_page == $i ? 'active' : ''; ?>">
                                                <a class="page-link" href="?id=<?php echo $council_id; ?>&events_page=<?php echo $events_page; ?>&awards_page=<?php echo $i; ?>"><?php echo $i; ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?php echo $awards_page >= $total_awards_pages ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?id=<?php echo $council_id; ?>&events_page=<?php echo $events_page; ?>&awards_page=<?php echo $awards_page + 1; ?>">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            <?php endif; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const councilId = <?php echo $council_id; ?>;
            ...
            function loadEventsPage(eventsPage, awardsPage) {
                ...
                const url = `?id=${councilId}&events_page=${eventsPage}&awards_page=${awardsPage}`;
                ...
                fetch(url)
                    ...
                    .then(html => {
                        ...
                        const newUrl = `?id=${councilId}&events_page=${eventsPage}&awards_page=${awardsPage}`;
