    ...
    if (isset($_POST['action'])) {
        ...
        switch ($_POST['action']) {
            case 'approve':
                ...
                $stmt = $conn->prepare("SELECT document_type FROM organization_documents WHERE id = ? AND organization_id = ?");
                ...
                if ($document) {
                    ...
                    $stmt = $conn->prepare("UPDATE organization_documents SET status = 'approved', adviser_approved_at = NOW(), approved_by_adviser = ? WHERE id = ? AND organization_id = ?");
                    $stmt->bind_param("iii", $_SESSION['user_id'], $documentId, $orgId);
                    
                    if ($stmt->execute()) {
                        // Create notification for president
                        require_once 'includes/notification_helper.php';
                        notifyAdviserDocumentAction($orgId, 'approved', $document['document_type'], $documentId);
                        
                        // Notify OSAS immediately when document is sent to them
                        notifyIndividualDocumentSentToOSAS($orgId, $document['document_type'], $documentId);
            ...
            case 'reject':
                if (empty($rejectionReason)) {
                    ...
                    if ($document) {
                        ...
                        $stmt = $conn->prepare("UPDATE organization_documents SET status = 'rejected', adviser_rejected_at = NOW(), approved_by_adviser = NULL, rejection_reason = ? WHERE id = ? AND organization_id = ?");
                        $stmt->bind_param("sii", $rejectionReason, $documentId, $orgId);
                        
                        if ($stmt->execute()) {
                            // Create notification for president
                            require_once 'includes/notification_helper.php';
                            notifyAdviserDocumentAction($orgId, 'rejected', $document['document_type'], $documentId);
        ...
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest') {
            ...
            header('Location: adviser_view_documents.php');
            exit;
        }
    }
}
...
$query = "
    SELECT d.*, u.username as submitted_by_name, u.email as submitted_by_email,
           d.rejection_reason, d.adviser_approved_at, d.adviser_rejected_at, d.osas_approved_at, d.osas_rejected_at,
...
$approvedCount = count(array_filter($documents, fn($d) => $d['status'] === 'approved' && $d['adviser_approved_at'] && empty($d['osas_approved_at'])));
...
<main>
    <div class="container-fluid py-2">
        ...
        <div class="page-header">
            ...
            <p class="page-subtitle">View document submissions and their status for <strong><?php echo htmlspecialchars($organization['org_name']); ?></strong></p>
        </div>
                    ...
                    <div class="card-body">
                        <form method="GET" action="">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <a href="adviser_view_documents.php" class="btn btn-secondary">
                                ...
                                <div class="row align-items-center">
                                    <div class="col-md-3">
                                        ...
                                        <span class="status-badge status-<?php echo $document['status']; ?>">
                                            ...
                                            if ($document['status'] === 'approved' && $document['adviser_approved_at'] && !empty($document['osas_approved_at'])) {
                                                echo 'Approved';
                                            } elseif ($document['status'] === 'approved' && $document['adviser_approved_at']) {
                                                echo 'Sent to OSAS';
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <?php elseif ($document['status'] === 'approved' && $document['adviser_approved_at'] && !empty($document['osas_approved_at'])): ?>
                                            <span class="badge bg-success">Approved</span>
                                            <small class="d-block text-muted">OSAS approved on <?php echo date('M d, Y', strtotime($document['osas_approved_at'])); ?></small>
                                        <?php elseif ($document['status'] === 'approved' && $document['adviser_approved_at']): ?>
                                            <span class="badge bg-success">Sent to OSAS</span>
                                            <small class="d-block text-muted">Approved on <?php echo date('M d, Y', strtotime($document['adviser_approved_at'])); ?></small>
                                        <?php elseif ($document['status'] === 'rejected' && $document['adviser_rejected_at']): ?>
                                            ...
                                            <?php if ($document['rejection_reason']): ?>
                                                <button type="button" 
                                                        ...
                                                        onclick="showRejectionReason('<?php echo htmlspecialchars($document['rejection_reason']); ?>')">
...
<script>
    ...
    document.addEventListener('submit', function(e) {
        ...
        fetch('adviser_view_documents.php', {
            method: 'POST',
