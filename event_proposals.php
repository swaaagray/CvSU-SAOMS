...
// Organization-only context
$orgId = getCurrentOrganizationId();
if (!$orgId) {
    echo '<div class="alert alert-danger">No organization found for this user.</div>';
    exit;
}
...
// --- New: cleanup ended semesters to remove last-semester event proposals and documents ---
function cleanupEndedSemesters($conn) {
    $today = date('Y-m-d');
    try {
        $semStmt = $conn->prepare("SELECT id FROM academic_semesters WHERE end_date < ? AND status != 'archived'");
        $semStmt->bind_param('s', $today);
        $semStmt->execute();
        $semRes = $semStmt->get_result();
        $semStmt->close();

        while ($semRow = $semRes->fetch_assoc()) {
            $semId = (int)$semRow['id'];
            // Start transaction for safe cleanup
            $conn->begin_transaction();
            try {
                // Get all approvals for this semester
                $approvalStmt = $conn->prepare("SELECT id FROM event_approvals WHERE semester_id = ?");
                $approvalStmt->bind_param('i', $semId);
                $approvalStmt->execute();
                $approvalRes = $approvalStmt->get_result();
                $approvalStmt->close();

                $deletedDocsTotal = 0;
                $deletedApprovalsTotal = 0;

                while ($app = $approvalRes->fetch_assoc()) {
                    $approvalId = (int)$app['id'];

                    // Fetch documents for this approval
                    $docStmt = $conn->prepare("SELECT id, file_path FROM event_documents WHERE event_approval_id = ?");
                    $docStmt->bind_param('i', $approvalId);
                    $docStmt->execute();
                    $docRes = $docStmt->get_result();
                    $docStmt->close();

                    // Delete files and DB rows per document
                    while ($doc = $docRes->fetch_assoc()) {
                        ...
                        $delDocStmt = $conn->prepare("DELETE FROM event_documents WHERE id = ?");
                        $delDocStmt->bind_param('i', $docId);
                        $delDocStmt->execute();
                        $deletedDocsTotal += $delDocStmt->affected_rows;
                        $delDocStmt->close();
                    }

                    // Delete approval row
                    $delAppStmt = $conn->prepare("DELETE FROM event_approvals WHERE id = ?");
                    $delAppStmt->bind_param('i', $approvalId);
                    $delAppStmt->execute();
                    $deletedApprovalsTotal += $delAppStmt->affected_rows;
                    $delAppStmt->close();
                }

                // Mark semester archived to avoid repeated cleanup
                $updSemStmt = $conn->prepare("UPDATE academic_semesters SET status = 'archived' WHERE id = ?");
                $updSemStmt->bind_param('i', $semId);
                $updSemStmt->execute();
                $updSemStmt->close();

                $conn->commit();
                error_log("cleanupEndedSemesters: Semester {$semId} cleaned: {$deletedApprovalsTotal} approvals, {$deletedDocsTotal} documents deleted.");
            } catch (Exception $e) {
                $conn->rollback();
                error_log("cleanupEndedSemesters: Failed to clean semester {$semId}: " . $e->getMessage());
                // continue to next semester
            }
        }
    } catch (Exception $e) {
        error_log("cleanupEndedSemesters: Query failed: " . $e->getMessage());
    }
}
// --- end new code ---

// Call cleanup on page load
cleanupEndedSemesters($conn);
        ...
        switch ($_POST['action']) {
            case 'submit_proposal':
				...
				if (!empty($title) && !empty($venue)) {
					...
					$stmt = $conn->prepare("INSERT INTO event_approvals (organization_id, semester_id, title, venue) VALUES (?, ?, ?, ?)");
					$stmt->bind_param("iiss", $orgId, $activeSemesterId, $title, $venue);
                    
                    if ($stmt->execute()) {
                        $approvalId = $stmt->insert_id;
                        
                        // Notify organization adviser about new event proposal submission
                        require_once 'includes/notification_helper.php';
                        notifyEventSubmission($orgId, $title, $approvalId);
...
$count_query = "SELECT COUNT(*) FROM event_approvals a WHERE a.organization_id = ?";
$count_stmt = $conn->prepare($count_query);
$count_stmt->bind_param("i", $orgId);
$count_stmt->execute();
...
// Get event proposals with approval status for organization only
$query = "
    SELECT a.*, 
...
if ($stmt) {
    $stmt->bind_param("iii", $orgId, $per_page, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
