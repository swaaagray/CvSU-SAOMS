...
requireRole(['council_president', 'council_adviser']);
...
// Handle form submission - only allow council presidents to make changes
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $role === 'council_president') {
    ...
    if (!empty($facebook_link) && !filter_var($facebook_link, FILTER_VALIDATE_URL)) {
        ...
        header('Location: council_settings.php');
        exit();
    }
    
    $logo_path = $council['logo_path']; // Keep existing logo by default
    ...
    if (isset($_FILES['logo']) && $_FILES['logo']['error'] === UPLOAD_ERR_OK) {
        ...
        $upload_dir = 'uploads/logos/councils/';
        ...
        $new_filename = 'council_' . $councilId . '_' . time() . '.' . $file_extension;
        $upload_path = $upload_dir . $new_filename;
        
        // Move uploaded file
        if (move_uploaded_file($_FILES['logo']['tmp_name'], $upload_path)) {
            // Delete old logo if it exists
            if (!empty($council['logo_path']) && file_exists($council['logo_path'])) {
                unlink($council['logo_path']);
            }
    ...
    $stmt = $conn->prepare("
        ...
        WHERE id = ?
    ");
    $stmt->bind_param("sssi", $description, $facebook_link, $logo_path, $councilId);
    
    if ($stmt->execute()) {
        $_SESSION['success_message'] = 'Council profile updated successfully!';
    } else {
        $_SESSION['error_message'] = 'Failed to update council profile';
    }
...
// Refresh council data with college info for display
$stmt = $conn->prepare("
    SELECT c.*, co.name as college_name 
    FROM council c
    LEFT JOIN colleges co ON c.college_id = co.id
...
<head>
    ...
    <title>Council Settings - CvSU Academic Organizations</title>
...
<body>
    ...
    <main>
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="page-title">Council Settings</h2>
                    <p class="page-subtitle">Configure your council details and preferences</p>
                </div>
                <div>
                    ...
                    <div class="settings-card">
                        <div class="settings-header">
                            ...
                            <p class="mb-0 opacity-75">
                                <?php echo htmlspecialchars($council['college_name'] ?? 'N/A'); ?>
                            </p>
                        </div>
                        
                        <div class="card-body p-4">
                            <?php if ($role === 'council_adviser'): ?>
                                <!-- View-Only Interface for Council Advisers -->
                                <div class="alert alert-info mb-4" role="alert">
                                    <i class="bi bi-eye me-2"></i>
                                    <strong>View Only Mode:</strong> You are viewing the council settings in read-only mode. Only the council president can modify these settings.
                                </div>

                                <!-- Logo Display Section -->
                                <div class="mb-4">
                                    <h6 class="form-label mb-3 text-center">
                                        <i class="bi bi-image me-2"></i>Council Logo
                                    </h6>
                                    <div class="text-center">
                                        <div class="mb-3">
                                            <?php if (!empty($council['logo_path']) && file_exists($council['logo_path'])): ?>
                                                <img src="<?php echo htmlspecialchars($council['logo_path']); ?>" 
                                                     alt="Council Logo" 
                                                     class="logo-preview">
                                            <?php else: ?>
                                        ...
                                        <div class="view-only-info">
                                            ...
                                            <?php else: ?>
                                                ...
                                                <small class="text-muted">Council has no logo set</small>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4">
                                    <h6 class="form-label mb-3">
                                        <i class="bi bi-text-paragraph me-2"></i>Council Description
                                    </h6>
                                    <?php if (!empty($council['description'])): ?>
                                        <div class="description-display">
                                            <?php echo nl2br(htmlspecialchars($council['description'])); ?>
                                        </div>
                                        <small class="text-muted mt-2 d-block">
                                            <i class="bi bi-info-circle me-1"></i>
                                            Character count: <?php echo strlen($council['description']); ?>/1000
                                        </small>
                                    <?php else: ?>
                                        ...
                                        <?php if (!empty($council['facebook_link'])): ?>
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <span class="text-muted small">
                                                    <?php echo htmlspecialchars($council['facebook_link']); ?>
                                                </span>
                                            </div>
                                        <?php else: ?>
                                    ...
                                    <div class="view-only-content">
                                        <div class="d-flex align-items-center gap-2">
                                            <button class="btn btn-outline-secondary btn-sm" 
                                                    onclick="window.open('council/<?php echo $councilId; ?>', '_blank')">
                            ...
                            <?php else: ?>
                                <!-- Editable Interface for Council Presidents -->
                                <form method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <textarea class="form-control" 
                                                  ...
                                                  rows="5" 
                                                  maxlength="1000"
                                                  placeholder="Enter a brief description of your council..."><?php echo htmlspecialchars($council['description'] ?? ''); ?></textarea>
                                        <div class="d-flex justify-content-between mt-2">
                                            <small class="text-muted">
                                                This will be displayed on your public council profile
                                            </small>
                                            <span class="char-counter">
                                                <span id="charCount"><?php echo strlen($council['description'] ?? ''); ?></span>/1000
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input type="url" 
                                               ...
                                               id="facebook_link" 
                                               name="facebook_link" 
                                               value="<?php echo htmlspecialchars($council['facebook_link'] ?? ''); ?>"
                                               placeholder="https://facebook.com/yourpage">
                                        <small class="text-muted d-block mt-2">
                                            Enter the full URL to your council's Facebook page
                                        </small>
                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <small class="text-muted d-block mt-2">
                                            This is your council's public profile page
                                        </small>
                                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Only initialize form functionality for council presidents
